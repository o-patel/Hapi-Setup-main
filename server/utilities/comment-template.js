...
const commentTemplate = configObj => {
    console.log('configObj: ', configObj.comments[0].requestType);
    let type = ''
    if(configObj.comments[0].requestType === 'LEAVE'){
        type = 'Leave'
        console.log('type: ', type);
    }
    if(configObj.comments[0].requestType === 'WFH'){
        type = 'Work from home'
    }
    if(configObj.comments[0].requestType === 'OVERTIME'){
        type = 'Overtime'
    }
    let header = `
    <!DOCTYPE html>
...
<td style="padding: 0px 35px">
    <p style="font-size: 22px; margin: 0px 0px 18px"> <b> Comments on ${type} from <span
                style="color:#1ed760">${configObj.commentedBy}</span></b></p>
