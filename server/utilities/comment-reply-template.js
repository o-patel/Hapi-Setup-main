...
const taskCommentTemplate = configObj => {
...
<td style="padding: 0px 35px">
    <p style="font-size: 22px; margin: 0px 0px 18px"> <b> Comments on Task -  <span
                style="color:#1ed760">${configObj.taskName}</span></b></p>
    ...
    if (configObj && configObj.comments && configObj.comments.length) {
        for (let index = 0; index < configObj.comments.length; index++) {
            ...
            const comment = `<p style="margin: 5px 0px;"> <b> ${element.messageBy.name}</b> : ${element.message} <br> </p>`
            let replyComment = ''
